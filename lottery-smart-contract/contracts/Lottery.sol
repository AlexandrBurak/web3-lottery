...
contract Lottery {
    ...
    uint256 public constant maxTickets = 100; // maximum tickets per lottery.ts
    uint256 public constant ticketCommission = 0.001 ether; // commition per ticket
    uint256 public constant duration = 20 minutes; // The duration set for the lottery.ts

    uint256 public expiration; // Timeout in case That the lottery.ts was not carried out.
    address public lotteryOperator; // the crator of the lottery.ts
    uint256 public operatorTotalCommission = 0; // the total commission balance
    address public lastWinner; // the last winner of the lottery.ts
    uint256 public lastWinnerAmount; // the last winner amount of the lottery.ts
    ...
    // modifier to check if caller is the lottery.ts operator
    modifier isOperator() {
        require(
